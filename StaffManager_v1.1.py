...
def main():
  ...
  try: 
    ...
    if (ch=='a' or ch=='A'):
        ...
        print("\n\n#############################################################\n")
        print("##\n")
        print("##   A. Fetch using id\n")
        print("##   B. fetch using name \n")
        ...
        if(ch1=='a' or ch1=='A'):
            id_string=int(input("\n\n\t\tEnter Staff ID: "))
            cur.execute("select * from staff_list where id={}".format(id_string))
            id_string=cur.fetchall()
            print("\n\n\t",id_string)
        elif(ch1=='b' or ch1=='B'):
            fn_string=input("\n\n\t\tEnter Staff first name: ")
            cur.execute("select * from staff_list where fname='{}'".format(fn_string))
            fn_string=cur.fetchall()
            print("\n\n\t",fn_string)
        elif(ch1=='m' or ch1=='M'):
            main()
        else:
        ...
        print("\n\nFor the sake of simplicity of program it is a made in a way where changing")
        print("\nthe column names and sizes can lead program to error full states and hence not")
        print("\nnot to work as desired, kindly only run SELECT Commands here..\n")
        print("\n     \tTable name : staff_list\n")
        print("\t|   id   | fname | lname |  mob  | email |  des  |")
        sql_string=input("\n\n\tEnter SQL Command without ; at last: ")
        cur.execute(sql_string)
        sql_string=cur.fetchall()
        print("\n\n\t",sql_string)
    elif (ch=='e' or ch=='E'):
        prog_run=False
        db.close()
